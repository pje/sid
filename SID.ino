...
void setup() {
  ...
  sid_set_ad_envelope(0, 0, 15);
  sid_set_sr_envelope(0, 0, 0);
  sid_set_volume(15);
      ...
      switch(opcode) {
        case MIDI_NOTE_ON :
          ...
          if (note_number < 96) { // SID can't handle freqs > B7
            ...
            sid_set_voice_frequency(0, MIDI_NOTES_TO_FREQUENCIES[note_number]);
            sid_set_gate(0, true);
            // sid_set_volume((note_velocity * 15) / 127);
          }
          break;
        case MIDI_NOTE_OFF :
          while (Serial1.available() <= 0) {
